...
class SparseTrainingArguments:
    ...
    layerwise_thresholds: str = field(
        default="d_1_h_1-d_1_h_1-d_1_h_1-d_1_h_1-d_1_h_1",
    ...
    def hybrid(cls, regularization_lambda):
        ...
        #sparse_args.layerwise_threshold
...
class ModelPatchingCoordinator:
    ...
    def __init__(self, sparse_args, device, cache_dir, model_name_or_path, logit_names, teacher_constructor):
        ...
        self.attention_heads_num=config.num_attention_heads
        self.layerwise_thresholds={}
    ...
    def _schedule_threshold(self,sparse_args,final_threshold,training,step,total_step,warmup_steps,compile):
        ...
        return threshold,regu_lambda,ampere_temperature,use_scheduler,mul_coeff

    def parse_layerwise_sparsity(self):
        layerwise_thresholds_str=self.sparse_args.layerwise_thresholds.split('-')
        layerwise_thresholds={}
        layerindex=0
        for item in layerwise_thresholds_str:
            layerwise_thresholds[layerindex]={}
            layerwise_thresholds[layerindex]['head']=float(item.split('_')[1])
            layerwise_thresholds[layerindex]['ffn']=float(item.split('_')[-1])
            layerindex+=1
        self.layerwise_thresholds=layerwise_thresholds
        return layerwise_thresholds
    ...
    def schedule_threshold(
        ...
        sparse_args = self.sparse_args
       # print('sparse_args',sparse_args)
        layerwise_thresholds= self.parse_layerwise_sparsity()
      #  print(layerwise_thresholds)
        initial_threshold = sparse_args.initial_threshold
        #final_threshold = sparse_args.final_threshold
        #attention_threshold=sparse_args.attention_threshold
        initial_warmup = sparse_args.initial_warmup
        final_warmup = sparse_args.final_warmup
        final_lambda = sparse_args.regularization_final_lambda
        initial_ampere_temperature = sparse_args.initial_ampere_temperature
        ...
        #print('sparse args',sparse_args,'compile',compile)
       # print('current step',step)
       
        context_data={}
        for layer in layerwise_thresholds:
            attention_threshold=layerwise_thresholds[layer]['head']
            ffn_threshold=layerwise_thresholds[layer]['ffn']
            threshold_head,regu_lambda_head,ampere_temperature_head, user_scheduler,mul_coeff_head=self._schedule_threshold(sparse_args,attention_threshold,training,step,total_step,warmup_steps,compile)
            threshold_ffn,regu_lambda_ffn,ampere_temperature_ffn, user_scheduler,mul_coeff_ffn=self._schedule_threshold(sparse_args,ffn_threshold,training,step,total_step,warmup_steps,compile)
            context_data[layer]={}
            context_data[layer]=dict(
            threshold_ffn=threshold_ffn,
            regu_lambda_ffn=regu_lambda_ffn,
            ampere_temperature_ffn = ampere_temperature_ffn,
            threshold_attention=threshold_head,
            regu_lambda_attention=regu_lambda_head,
            ampere_temperature_attention = ampere_temperature_head,
            progress_ffn = 1.0 - mul_coeff_ffn,   
            progress_attention = 1.0 - mul_coeff_head
        )
        '''
        if not training:
            step -= 1
        ...
        #    regu_lambda=regu_lambda_linear,
         #   attention_threshold=threshold_attention,
       ...
       # print('context_data',context_data,step)
    ...
    def patch_model(self, model, trial = None):
        ...
        #print('attention_pruning_method_parts',attention_pruning_method_parts)
        ...
        #print('bias mask',bias_mask,'linear_min_parameters',linear_min_parameters)
        ...
        if attention_pruning_method_parts[0] != "disabled" or sparse_args.ampere_pruning_method != "disabled":
           ...
           # print('args attention',args_attention)
            #print('args attention t',args_attention_t)
         #   print('args attention submethod',args_attention.submethod)
            if args_attention.submethod == "joint":
                ...
                #print('model strcuture',self.model_structure)
               # print('head mask',self.layer_head_mask)
       ...
       # print('module_patchers',module_patchers)
    ...
    def compile_model(self, model):
        ...
        removed_heads, total_heads = pruner.run(self.layerwise_thresholds,self.attention_heads_num)
        print(
