...
class SparseTrainer:
    ...
    def log(self, logs: Dict[str, float]) -> None:
        add = {self.log_prefix + str(k): str(v) for k, v in self.patch_coordinator.log().items()}
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        optimizer_type = self.optimizer or 'adamw'
        self.optimizer = self.create_optimizer(self.model, optimizer_type)

        lr_scheduler_type = self.lr_scheduler or 'linear'
        self.lr_scheduler = self.create_scheduler(num_training_steps, lr_scheduler_type)

        print(f'Using (optimizer, lr_scheduler) = ({optimizer_type}, {lr_scheduler_type})')

    def create_optimizer(self, model, optimizer_type='adamw'):
        args = self.args
    ...
    def create_scheduler(self, num_training_steps: int, lr_scheduler_type='linear'):
