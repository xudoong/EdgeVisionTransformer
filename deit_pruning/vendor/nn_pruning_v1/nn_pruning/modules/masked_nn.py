...
class BlockLinearPruningContextModule(LinearPruningContextModule):
    def __init__(self, shape, args: LinearPruningArgs):
        ...
        print('blocklineaer',self.mask_size)
...
class SingleDimensionLinearPruningContextModule(LinearPruningContextModule):
    def __init__(self, shape, is_row, args: LinearPruningArgs):
        ...
        print('singledimensionlinear mask size',size)
...
class MaskModule(nn.Module):
    ...
    def mask(
        ...
        if method == "topK":
            ...
            # mask = MagnitudeBinarizer.apply(mask_scores, threshold)
    ...
    def forward(self, weight, threshold,module_name): ## call mask
        mask_scores = None
        if self.args.method != "magnitude":
            mask_scores = [(c.mask_scores if c is not None else None) for c in self.context_modules]
        # else:
        #     mask_scores = [weight.clone()]
        #     print(mask_scores, mask_scores[0].shape)
        #     print([(c.mask_scores if c is not None else None) for c in self.context_modules], [(c.mask_scores if c is not None else None) for c in self.context_modules][0].shape)
        #     exit(0)
      #  print('mask linear forward',mask_scores)
        return self.mask(weight, mask_scores, self.args, threshold, self.training,module_name)
...
class MaskedLinear(ReplacementModule):
    ...
    def get_masked_weights_bias(self):   ## apply mask to weights
        #print(self.module_name)
#        if 'attention' in self.module_name and ('key' in self.module_name or 'value' in self.module_name or 'query' in self.module_name) :
        if 'attention' in self.module_name:
            threshold = self.get_context_data("threshold_attention",self.module_name)
        else:

            threshold = self.get_context_data("threshold_ffn",self.module_name)
        #threshold=self.get_context_data('threshold',self.module_name)
        #threshold=1.0
       # print(self.module_name,threshold)
        mask = self.mask_module(self.weight, threshold,self.module_name)
        #if 'attention' in self.module_name and threshold<1.0:
        #    ('get threshold',threshold,self.module_name)
       #     print('gettting mask score',mask)
    ...
    def forward(self, input):
        ...
        #print(self.module_name,masked_weights)
        # Compute output (linear layer) with masked weights
        return F.linear(input, masked_weights, bias)  ### input* masked_weights+bias
    ...
    def compile(self):
       ...
       # print('maskedliner compile',ret.shape)
        with torch.no_grad():
            ret.weight.copy_(masked_weights)
            if ret.bias is not None:
                ret.bias.copy_(bias)
       # print('maskedliner after compile',ret.shape)
        return ret
...
class LinearPruningModulePatcher(ModulePatcher):
    ...
    def create_context_module(self, child_module_name, child_module, key, row=None):
        ...
        if prefix == "mask":
           ...
           # print(child_module_name,'blocklinaer')
            return BlockLinearPruningContextModule(shape, args)

        elif prefix == "mask_1d":
            assert args.submethod.startswith("1d")
            assert row in [True, False]
          #  print(child_module_name,'singledimensionlinaer')
    ...
    def patch(self, child_module_name, child_module):
      #  print('here',child_module_name,child_module)
        if self.args.method != "disabled":
            if self.args.submethod.startswith("1d"):
                ...
                shape = child_module.weight.shape
                print(child_module_name,'mask_row',mask_row)
                ...
                print(child_module_name,'mask_context_module',mask_context_module)
        else:
        ...
        #print(child_module)
        return MaskedLinear(child_module_name,child_module, mask_context_modules, ampere_context_module, self.args,
