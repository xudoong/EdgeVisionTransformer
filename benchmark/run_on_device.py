...
def run_on_android(modelpath, adb, use_gpu=False, num_threads=1, num_runs=10, warm_ups=10, skip_push=False):
    ...
    command=f"taskset 10 /data/tf_benchmark/benchmark_model_plus_flex --num_threads={num_threads} {'--use_gpu=true' if use_gpu else ''} --num_runs={num_runs} --warm_ups={warm_ups} --graph="+'/sdcard/'+modelname
    print(command)
