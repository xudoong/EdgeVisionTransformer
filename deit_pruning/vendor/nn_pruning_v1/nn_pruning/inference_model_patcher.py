...
class BertHeadsPruner:
    def __init__(self, model):
        ...
        #self.num_attention_heads=count_num_heads(model)
    ...
    def get_pruned_heads(self,layerwise_thresholds,num_attention_heads):
        ...
        for name, module in self.model.named_modules():
            if name.endswith(self.attention_prefix):
                ...
                block_scores=[]
                #layers=0
                layer_attention_threshold=layerwise_thresholds[layer_number]['head']
                for a in self.attention_layers: ##query,key,value
                    p = self.analyze_head(getattr(module, a).weight, module.attention_head_size)
                   # print(layer_number,name,module,a,p)
                    parts.append(p)
                    ...
                    if len(block_scores)==0:
                        ...
                        block_scores=block_scores+p.int()
                ##original scoring
               ...
               # print('before',parts)
               # to behave the same as binarizer
                to_prune_head_num = num_attention_heads - int(layer_attention_threshold * num_attention_heads)
              #  print(to_prune_head_num,attention_threshold,num_attention_heads)
              #  print(to_prune_head_num,block_scores)
                _,indices=torch.sort(block_scores)
                to_prune_indices=indices[0:to_prune_head_num]
                parts=torch.ones(num_attention_heads,dtype=bool)
               # print(to_prune_indices,parts)
                for indice in to_prune_indices:
                    parts[indice.item()]=False
                #print('after parts',parts)
    ...
    def run(self,attention_threshold,num_attention_heads):
        model = self.model
        print('model here',model,self.model.config)
        if isinstance(self.model.config, ViTConfig):
            to_prune, heads_count = self.get_pruned_heads(attention_threshold,num_attention_heads)
            model.prune_heads(to_prune)
            return sum([len(p) for p in to_prune.values()]), heads_count
        print('Warning: model.config is not ViTConfig, skip prune_heads().')
...
def optimize_model(model, mode, clone=True):
    ...
    mp = InferenceModelPatcher(prune_heads=False, mode=mode) ## change, the default prune_heads=True
